...
export interface ModelAsset {
  /** 模型场景图。**每次使用都要 clone** —— 同一个模型可能被多个场景引用 */
  scene: THREE.Group;
 ...
 */
function applySamplerSettings(texture: THREE.Texture): void {
...
async function loadImage(key: AssetKey): Promise<THREE.Texture> {
  const loader = new THREE.TextureLoader();
  loader.setCrossOrigin('anonymous');
  const texture = await loader.loadAsync(resolveAsset(key));
  applySamplerSettings(texture);
  return texture;
}

async function loadKtx2(key: AssetKey, renderer: THREE.WebGLRenderer): Promise<THREE.Texture> {
  const loader = getKTX2Loader(renderer);
  const texture = await new Promise<THREE.Texture>((resolve, reject) => {
    loader.load(resolveAsset(key), resolve, undefined, reject);
  });
  applySamplerSettings(texture);
  return texture;
}

async function loadModel(key: AssetKey, renderer: THREE.WebGLRenderer): Promise<ModelAsset> {
  const loader = new GLTFLoader()
    .setDRACOLoader(getDracoLoader())
    .setKTX2Loader(getKTX2Loader(renderer));

  const gltf = await loader.loadAsync(resolveAsset(key));
...
export async function loadAssets(
  ...
  const textures = new Map<AssetKey, THREE.Texture>();
  const models = new Map<AssetKey, ModelAsset>();
  ...
  await Promise.all(
    unique.map(async (key) => {
      ...
      try {
        if (kind === 'glb') {
          models.set(key, await loadModel(key, renderer));
        } else if (kind === 'ktx2') {
          textures.set(key, await loadKtx2(key, renderer));
        } else {
          textures.set(key, await loadImage(key));
        }
      } catch (err) {
        ...
        const hint =
          kind === 'ktx2' || kind === 'glb'
            ...
            : '';
        throw new Error(
          `[loaders] 素材加载失败: ${key} (kind=${kind}, path=${resolveAsset(key)})${hint}\n` +
